...
void myrecv()
{
	do
	{
		...
		recvlen = read(sockfd, buf, buffer_len);
	}
	while(recvlen == 0);
	buf[recvlen] = 0;
	printf(buf);
...
int d_init()
{
	...
	else
	{
	  ...
	  d_saddr.sin_port = htons(dataPort);
		...
		if(connect(d_sockfd, (struct sockaddr*)&d_saddr, sizeof(struct sockaddr_in)) < 0)
		{		
...
#define d_buffer_len 60000
char d_buf[d_buffer_len + 1];
...
void d_myrecv(FILE *d_f)
{
	while(1)
	{	
		recvlen = read(d_sockfd, d_buf, d_buffer_len);		
