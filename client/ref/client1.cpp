...
//    printf("Recv: %s", buf);
    ...
    while (ret) {
        ...
        if (i == 4)strcat(data_ip, ret);
        if (i == 5)data_port += atoi(ret)*256;
        if (i == 6)data_port += atoi(ret);
        i ++;
        ret = nextToken(NULL);
    }
    ...
    memset(buf,0,BUFFERSIZE);
    if (connect(s_sockfd, (struct sockaddr *)&their_data_addr, sizeof(struct sockaddr)) == -1) {
...
int main(int argc, char *argv[]) {
	//@TODO 把登陆作为程序启动后的命令来做
    int sockfd, numbytes;
    char buf[BUFFERSIZE];
    char cDir[DIRLENGTH];

    if (argc != 6) {
        fprintf(stderr,"Usage: client -i <ip> <username> <password> <port>\n       client -h <hostname> <username> <password> <port>\n");
        exit(1);
    }
