...
#define SERVER_PORT 2437
int IP[4] = {166, 111, 68, 19};
...
void getRequest()
{
	req_len = read(clientSock, buf, buffer_len);
	buf[req_len] = 0;
	printf(buf);
}
...
{	
  ...
  d_myaddr.sin_port = htons(SERVER_PORT + 1000);
	...
	while(bind(d_sockfd, (struct sockaddr*)&d_myaddr, sizeof(struct sockaddr_in)) < 0)
	{
	...
	sprintf(temp, "227 Entering Passive Mode (%d,%d,%d,%d,%d,%d)\r\n",
	 IP[0], IP[1], IP[2], IP[3], ntohs(d_myaddr.sin_port) >> 8, ntohs(d_myaddr.sin_port) & 0xFF);	
	...
	d_clientSock = accept(d_sockfd, (struct sockaddr*)&d_clientaddr, &sockLength);
	if(d_clientSock < 0)
	{
		printf("Accept error.\n");
		return -1;
...
#define d_buffer_len 3000

char d_buf[d_buffer_len];
...
void d_mysend(FILE* d_f)
{
	if(d_f)
	{
		while(1)
		{
			int readlen = fread(d_buf, 1, d_buffer_len, d_f);
			if(readlen == 0)
				break;
...
void d_myreceive(FILE* d_f)
{
	while(1)
	{	
	  int	recvlen = read(d_clientSock, d_buf, d_buffer_len);
