...
void *thread(void *parameter) {
	...
	while (true) {
		...
		if (subBuff[0] == "PASV\r\n") {
			...
			int dataon = 1;
			if (setsockopt(datasock, SOL_SOCKET, SO_REUSEADDR, &dataon,
					sizeof(dataon)) < 0) {
				exit(0);
			}
			if (bind(datasock, (struct sockaddr*) &datasockip,
			...
			cout << command;
			listen(datasock, 10);
			int dataserver = accept(datasock, (struct sockaddr*) &dataclientIp,
					&datalen);
			memset(recvBuff, ' ', 1024);
			total = recv(*server, recvBuff, (int) strlen(recvBuff), 0);
			recvBuff[total] = '\0';
			...
			if (subBuff[0] == "RETR") {
				...
				if (stat(filename.substr(0, filename.size() - 2).c_str(), &st)
						!= 0) {
					close(dataserver);
					close(datasock);
					strcpy(command, "500\r\n");
			...
			if (subBuff[0] == "CWD\r\n") {
				...
				if (subBuff[1][0] == '/') {
					DIR *dir;
					dir = opendir((serverhomedir + subBuff[1].substr(0,
							subBuff[1].size() - 2)).c_str());
					if (dir != NULL) {
						servercurrentdir = serverhomedir + subBuff[1].substr(0,
								subBuff[1].size() - 2);
						clientcurrentdir = subBuff[1].substr(0,
								subBuff[1].size() - 2);
						strcpy(command, "250\r\n");
						command[5] = '\0';
						send(*server, command, strlen(command), 0);
					...
					if ((servercurrentdir == "/media/sda6" && clientcurrentdir
							== "") || (servercurrentdir == "/media/sda6/"
							&& clientcurrentdir == "/")) {
					} else {
						int tempindex = clientcurrentdir.find_last_of('/');
						clientcurrentdir
								= clientcurrentdir.substr(0, tempindex);
					...
					if ((servercurrentdir == "/media/sda6/" && clientcurrentdir
							== "/")) {
						newserverdir = servercurrentdir + subBuff[1].substr(0,
								subBuff[1].size() - 2);
						newclientdir = clientcurrentdir + subBuff[1].substr(0,
								subBuff[1].size() - 2);
					} else {
						newserverdir = servercurrentdir + "/"
								+ subBuff[1].substr(0, subBuff[1].size() - 2);
						newclientdir = clientcurrentdir + "/"
								+ subBuff[1].substr(0, subBuff[1].size() - 2);
					}
